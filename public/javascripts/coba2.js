(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
  1: [function (require, module, exports) {
  ...
  }, { "./build/conversion.js": 32 }], 34: [function (require, module, exports) {
    // const socket = io()
    ...
    const ecekeditor = document.getElementById('editor')
    ...
    const paragraph = element.children
    // paragraph.classList.add('test')
    // console.log(paragraph[0]);
    Array.from(paragraph).forEach(par => {
      const img = par.firstElementChild
      if (img) {
    ...
    CKEDITOR.replace('editor', {
      ...
      "imageBrowser_listUrl": "/coba/files"
    });
    ...
    tombol1.addEventListener('click', async () => {
      const dataCkeditor = CKEDITOR.instances.editor.getData()
      console.log(typeof (dataCkeditor));
      // console.log(ecekeditor.value);
    })
    ...
    const tampilanPesan = message => {
      message.forEach(mess => {
        element.innerHTML += `
            <div class="uppercase font-bold list px-56 ">
              <div class="text-center mb-3 ">
                ${mess.judul}
                ${mess.id}
              </div>
              <div>
                <div class="mt-1 ml-8 font-normal normal-case ">
                  ${mess.description}
                </div>
              </div>
    ...
    window.onload = function () {
      const tombolDelete = document.querySelectorAll('button.tombolDelete')
    ...
    window.uploadBanner = async () => {
      ...
      await Axios.post('http://localhost:2500/coba/upload', formData, config)
        .then(response => {
          bannerPath = `${response.data}`
          bannerImg.src = `${bannerPath}`
        })
      // banner.style.backgroundImage = `url("${response.data}")`
    ...
    if (blogForm) {
      blogForm.addEventListener('submit', async (e) => {
        ...
        const image = bannerPath
        // fd.append('upload', blogForm['blogFoto'].files[0])
        // fd.upload = blogForm['blogFoto'].files[0]
        // fd.judul = blogForm['judul'].value
        // fd.append('description', blogForm['description'].value)
        // fd.append('judul', blogForm['judul'].value)
        // console.log(fd);

        await Axios.post('http://localhost:2500/coba/uploadBlog', { judul, description, image }, {
          headers: {
            Authorization: `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE2NjA4OTU0NTQsImV4cCI6MTY2MjYyMzQ1NH0.2k-DBSHsdF3ALBXkKz2zkl2WtwqUMvXxtWxX419Zx3Q`
          }
        })
          .then(response => {
            CKEDITOR.instances.editor.setData("")
            blogForm['judul'].value = ""
            bannerImg.src = ``
            // console.log(response.request.responseURL);
            const message = response.data.message
            const bg = response.data.bg
            window.location.href = `http://localhost:2500/coba/admin?message=${message}&bg=${bg}&cur=0`
            // location.href = "http://localhost:2500/";
          })
        // if (idBlog.value) {
    ...
    window.loadFile = (event) => {
      // const file = document.getElementById('blogFoto').files[0];
      // console.log(file);
      // imageConversion.compressAccurately(file, 100).then(res => {
      //   //The res in the promise is a compressed Blob type (which can be treated as a File type) file;
      //   console.log(res);
      // })
      const data = event.target.files.length;
      const gambar = document.getElementById('gambar')
      const img = document.createElement("img");
      const output = document.getElementById('output')
      gambar.removeChild(gambar.firstChild)
      img.setAttribute("class", "mt-4");
      // multiple image
      // for (let i = 0; i < data; i++) {
      // img.setAttribute("src", URL.createObjectURL(event.target.files[i]));
      // gambar.appendChild(img.cloneNode(true))
      // }
      // img.onload = function () {
      ...
      // output.setAttribute("class", "mt-4");
    ...
    tombol.addEventListener('click', async () => {
      window.location.href = "http://localhost:2500/coba/admin?flash=semuaOke"
    })

  }, { "axios": 2, "image-conversion": 33 }]
}, {}, [34]);
