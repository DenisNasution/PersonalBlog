    ...
    1: [function (require, module, exports) {
        ...
        const element = datas => {
            datas.forEach(data => {
                bodyTable.innerHTML += `
                <tr>
                    <td class="border-b-2 border-slate-400 text-left py-3">
                        ${data.id}
                    </td>
                    <td class="border-b-2 border-slate-400 pl-6 text-left py-3">
                        ${data.kategori}
                    </td>                   
                    <td class="border-b-2 border-slate-400 text-center py-3">
                        <a onclick ="ondelete(${data.id})">
                            <i class="fas fa-trash"></i>
                        </a>
                        <a onclick="onedit(${data.id})" >
        ...
        window.ondelete = async (id) => {
            ...
            if (result == true) {
                await Axios.delete(`http://localhost:3000/coba/adminCategory/delete/${id}`)
                    .then(response => {
        ...
        categoryForm.addEventListener('submit', async (e) => {
            ...
            const desk = deskripsi.value
            const id = idKategori.value
            // console.log(id);
            if (id) {
                await Axios.put('http://localhost:3000/coba/adminCategory/edit', { kat, desk, id })
                    .then(response => {
                        kategori.value = ""
                        deskripsi.value = ""
                        idKategori.value = ""
        ...
        <div>
            <i class="fas fa-times"></i>
                ...
                await Axios.post('http://localhost:3000/coba/adminCategory', { kat, desk })
                    .then(response => {
                        kategori.value = ""
                        deskripsi.value = ""
                        alert.classList.add(response.data.bg)
        ...
        const getDataAdminCategory = async () => {
            await Axios.get('http://localhost:3000/coba/getDataAdminCategory')
                .then(response => {
                    ...
                    function myGreeting() {
                        ...
                        const url = "http://localhost:3000/coba/adminCategory?message=Upload%20Berhasil&bg=bg-green-300"
        ...
        window.onedit = async (id) => {
            await Axios.get(`http://localhost:3000/coba/adminCategory/${id}`)
                .then(response => {
                    kategori.value = response.data[0].kategori
                    deskripsi.value = response.data[0].deskripsi
                    idKategori.value = response.data[0].id
