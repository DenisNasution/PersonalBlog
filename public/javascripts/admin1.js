...
(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
    1: [function (require, module, exports) {
        ...
        const tombolAdmin = document.querySelector('#tombolAdmin')
        const tampilPesan = document.getElementById('tampilPesan')
        const idBlog = document.getElementById('idBlog')
        ...
        const element = datas => {
            datas.forEach(data => {
                bodyTable.innerHTML += `
                <tr>
                    <td class="border-b-2 border-slate-700 text-left py-3">
                        ${data.id}
                    </td>
                    <td class="border-b-2 border-slate-700 text-left py-3">
                        ${data.judul}
                    ...
                    <td class="border-b-2 border-slate-700 py-3 w-1/4">
                        <img src="${data.image}" width="80" height="80" class="mx-auto" />
                    </td>
                    <td class="border-b-2 border-slate-700 text-center py-3">
                        <a href="/coba/viewAdmin/${data.id} ">
                            <i class="fas fa-eye" id="viewIcon" data-id="${data.id}"></i>
                        </a>
        ...
        window.changeKat = () => {
            ...
            getDataAdmin(kategori.value)
        }

        window.ondelete = async (id) => {
            const result = confirm('are you sure?')
            if (result == true) {
                await Axios.delete(`http://localhost:2500/coba/delete/${id}`)
                    .then(response => {
                        alert.classList.add(response.data.bg)
                        alert.innerHTML = `<h4>${response.data.message}</h4>
                    <div>
                        ...
                        function myGreeting() {
                            ...
                            alert.innerHTML = ""
                            alert.classList.remove(response.data.bg)
                        }
                    })
            } else {
                return false;
        ...
        const getDataAdmin = async (kategori) => {
            // console.log(kategori);
            await Axios.post('http://localhost:2500/coba/getDataAdmin', { kategori })
                .then(response => {
                    ...
                    function myGreeting() {
                        ...
                        // const url = "http://localhost:2500/coba/admin?message=Upload%20Berhasil&bg=bg-green-300"
                        // function getPathFromUrl(url) {
                        //     return url.split(/[?#]/)[0];
                        // }
                        // flash.classList.remove(response.data.bg)
                    }
                })
        }
        getDataAdmin()
        ...
        //     await Axios.get(`http://localhost:2500/coba/getData/${idBlog.value}`)
        //         .then(response => {
        //             // console.log(response.data);
        //             judul.value = response.data[0].judul
        //             CKEDITOR.instances.editor.setData(response.data[0].description)
        //             bannerImg.src = response.data[0].image
        //         })
        // }
        // getData()
        ...
        //         location.href = "http://localhost:2500/coba/edit"
        //         await Axios.get(`http://localhost:2500/coba/getData/${id}`)
        //             .then(async (response) => {
        //                 tampilPesan()
        //             })
        //     })
        ...
        tombolAdmin.addEventListener('click', () => {
            console.log(idBlog.value);
        })
